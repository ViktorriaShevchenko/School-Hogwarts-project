...
* Интеграционные тесты с TestRestTemplate.
* ВНИМАНИЕ: Эти тесты работают с реальной БД и требуют:
1. Отдельной тестовой БД ИЛИ
2. Настройки сброса последовательности ID ИЛИ
3. Использования H2 in-memory БД для тестов

Для упрощения в учебных целях некоторые тесты отключены.
